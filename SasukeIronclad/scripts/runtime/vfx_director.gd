...
func _spawn_sparks(root: Node2D, params: Dictionary) -> void:
    ...
    for index in range(count):
        ...
        spark.points = PackedVector2Array([
            Vector2.ZERO,
            Vector2.RIGHT.rotated(angle) * length
        ])
        spark.width = 2.5
...
func _spawn_vertical_lightning_slash(root: Node2D, params: Dictionary) -> void:
    ...
    slash.points = PackedVector2Array([
        Vector2(0, -height * 0.55),
        Vector2(8, -height * 0.12),
        Vector2(-4, height * 0.25),
        Vector2(2, height * 0.55)
    ])
    slash.width = width
...
func _spawn_focused_lightning_pillar(root: Node2D, params: Dictionary) -> void:
    ...
    core.points = PackedVector2Array([
        Vector2(0, 34),
        Vector2(-8, -height * 0.25),
        Vector2(7, -height * 0.58),
        Vector2(0, -height)
    ])
